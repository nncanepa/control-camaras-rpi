...
class RPCHandler:
    ...
    def capturar(self):
        ...
        imagen = capturaRpi(np.random.randint(0,255, size=(500,1500,3)), self.cam.iso,
                            self.cam.shutter_speed, self.cam.framerate, 'camara1', crop=(self.xi,self.xf,self.yi,self.yf),
                            timestamp=timestamp)
        return imagen
...
rpc_server(handler, ('localhost', 5001), authkey=b'peekaboo')